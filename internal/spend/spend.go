...
// says whether the record exists, since one entry carries both.
...
// more precision than that, and printing every digit would claim an accuracy
...
//
// Millions and billions are tiers of their own because a roll-up can reach
// them, and a twelve-figure count written in thousands is not read at a
// glance. A single session can reach millions on its own.
