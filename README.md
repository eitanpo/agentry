...
| `--[no-]thinking\|tools\|tool-results\|subagents\|metrics` | render | — | Override a single channel on top of `--level` (adds or subtracts). `tools` = a tool fired; `tool-results` = its output, and on an `Agent` call the instruction it was handed — printed whole, above the result or the expanded subagent stream, behind the `❯` glyph a typed prompt carries. A result body is cut to its first ten lines with the remainder named (`… 38 more lines`), a single result being unbounded machine output; `--turn` prints bodies whole, having already narrowed to one turn — and each call's whole argument text as an `args` block beneath its line, wherever the line's parenthetical left something out. `--format json` carries them whole always. An `Agent` line also names what it delegated to: `Agent[Explore@haiku]` is the subagent type and the model, and the `@model` half is absent when the call left the subagent on the session's model. |
