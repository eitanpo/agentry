...
agentry search "learnings"                 # all lower case, so any casing matches
agentry search "Learnings"                # a capital asks for that casing exactly
agentry search -F "compilePattern("       # the pattern as text, punctuation and all
agentry search "load the skill" --format jsonl | jq  # one record per line, for piping
```

Across sessions, two nouns — turns, or the sessions holding them. Both take the listing's filters:

```
agentry search turn "unplaced tokens" --since 7d        # matching turns, grouped by session
agentry search session "unplaced tokens" --all-projects # sessions, and how many turns matched
agentry search turn "a phrase" --used-skill review      # only sessions that invoked that skill
agentry search turn "a phrase" --format jsonl \
  | jq -r '"\(.sessionId) --turn \(.payload.turn)"' \
  | xargs -L1 agentry view --level full                 # open every matching turn
```
...
Each matching turn is one block, headed by the turn number and how many of its lines matched, with up to
...
the number of the call or the prose block the line sits in — the field that tells two of them apart — and
the rendered turn prints that same number beside each call and above each prose block, so `call 7` and
`block 12` are numbers you look up rather than count to; one row would not fit,
...
match count reads down one edge, with the id last as on a listing row — the value you copy out never
moves — and the title taking whatever the fixed columns leave. It is drawn in the listing's own colors
too, the id's unique prefix emphasized by the same rule, so the two surfaces read as one row. Rows print oldest-to-newest, so
...
pattern you typed: all lower case matches any casing, one capital letter makes case matter. Text
carrying `(`, `[` or `.` needs `-F` to be read as itself.
...
| `--[no-]thinking\|tools\|tool-results\|subagents\|metrics` | render | — | Override a single channel on top of `--level` (adds or subtracts). `tools` = a tool fired; `tool-results` = its output, and on an `Agent` call the instruction it was handed — printed whole, above the result or the expanded subagent stream, behind the `❯` glyph a typed prompt carries. A result body is cut to its first ten lines with the remainder named (`… 38 more lines`), a single result being unbounded machine output, and a body of more than one line is numbered from 1 so a search hit reported at `result:66` names a line you can find; `--turn` prints bodies whole, having already narrowed to one turn — and each call's whole argument text as an `args` block beneath its line, wherever the line's parenthetical left something out. `--format json` carries them whole always. An `Agent` line also names what it delegated to: `Agent[Explore@haiku]` is the subagent type and the model, and the `@model` half is absent when the call left the subagent on the session's model. |
...
| `--reply-matches PATTERN` | `list` | — | Only sessions whose assistant reply text matched. PATTERN is a **regular expression** (RE2) read under smart case: all lower case matches any casing, a capital letter makes case matter, and `(?i)`/`(?-i)` override either way — the one filter here that is not a substring match, because prose questions are positional and alternation-shaped. Tested against each assistant text block separately, so `^`/`$` anchor to one reply: `'(?m)^\*{0,2}Learnings\b'` finds the block, where the substring `Learnings` would also hit every session that merely mentioned it. Thinking blocks and subagent sidecars are not read. An unparseable pattern is a usage error, and `-F` reads PATTERN as text instead. Reply text is matched but never printed or serialized — see the `--format json` note in [PRODUCT.md](PRODUCT.md) for the size reason. |
| `search turn PATTERN`, `search session PATTERN` | — | — | Search across sessions rather than inside one. `turn` prints matching turns grouped under a session heading; `session` prints those headings alone, each with how many turns matched. Both take the listing's selection flags (`--since`, `--until`, `--from`, `--project`, `--all-projects`, every `--used-*` and output filter) from the same table `list` uses. Neither is capped: a cap on which sessions are searched would hide matches the pattern asked for. A first positional of `turn` or `session` is the noun — the rule agentry already uses for verbs, and the plural of either reads the same way — so `agentry search turn` reports its pattern missing; search for one of those four words with `agentry search -- turn`. |
