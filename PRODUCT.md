...
- Add `agent-name` to the title ladder, above `custom-title`. Naming a session with `--name` or `/rename` appends an `agent-name` entry, and Claude Code usually mirrors that string into `custom-title` — so the ladder lands on the right title by duplication, not by reading the name. Where only `agent-name` was written, the listing silently falls through to the generated `ai-title` and shows a name you did not choose.
