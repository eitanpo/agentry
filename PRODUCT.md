...
- `--format json` with no Claude project for the directory: emit `[]` on stdout, keeping the stderr message and the non-zero exit. Today that case writes nothing to stdout while an empty *selection* writes `[]`, so one output contract has two shapes and every script that sweeps directories carries a guard to substitute the missing array before `jq` sees it.
