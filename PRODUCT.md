...
**Roadmap:** ordered by what is blocked today. The first group has a caller already working
around its absence; the second is where agentry shows something wrong rather than merely
omitting it; the third adds detail nothing currently asks for; the fourth is the long-standing
product surface, which no audit work depends on. Insert a new item into the group whose
description it matches rather than at the end.

- Listing beyond the current directory: `--all-projects`, and `--project <path>` to name one other than the cwd — where naming a repo sweeps its worktrees too. Every selector is scoped to the current directory today, so auditing an instruction layer shared across repos — skills symlinked into every project — means re-running the binary once per repo. The same gap bites inside a single repo, because Claude Code gives every git worktree its own project folder: `agentry list` in the main checkout shows none of the worktree sessions, and inside a worktree shows only that worktree's. Treat the two as one design; a `--project` built without deciding how it maps a repo to its worktree slugs gets built twice. That sweep also silently drops sessions whose project directory has since been deleted or renamed, and the on-disk slug cannot be reversed to recover the path.
